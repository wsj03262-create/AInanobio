...
class RGBApplianceGUI(QWidget):
    ...
    def _set_label_if_changed(self, label, text: str):
        if label.text() != text:
            label.setText(text)
    ...
    def _flush_csv_if_needed(self, force=False):
        try:
            if self.csv_file is not None and (force or self.pending_flush_count >= CSV_FLUSH_EVERY_N_ROWS):
                self.csv_file.flush()
                self.pending_flush_count = 0
        except Exception:
            pass
    ...
    def _update_counts_ui(self):
        self._set_label_if_changed(self.lab_img_count, f"이미지 수집: {self.image_count}")
        self._set_label_if_changed(self.lab_data_count, f"데이터 수집: {self.data_log_count}")
